...
<template>
    <section class="flex flex-col items-center justify-center w-full gap-[4rem] min-h-screen pt-[10rem]">
        ...
        <div class="flex flex-col w-full px-[4rem] pb-[5rem]">
            <div 
                ...
                :key="index" 
                :class="index % 2 === 1 ? 'justify-end flex-row' : 'justify-start flex-row-reverse'"
            >
                        ...
                        <div class="flex flex-col justify-center w-full gap-3 p-[2rem]" v-for="(post, indexes) in item.possition" :key="indexes">
                            <div class="flex items-start justify-between w-full font-bold tracking-wide text-md">
                                <div class="flex flex-col gap-2">
                                    <span class="flex items-center w-auto gap-2 text-xl"> <span class="p-1 bg-green-300"></span> <span>{{ post.icon }}</span></span>
                                    <div class="w-auto"><small class="px-3 py-1 text-black capitalize bg-green-300 rounded-full">{{post.color}}</small></div>
                                </div>
