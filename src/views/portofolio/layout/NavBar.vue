...
<template>
    <nav class="fixed z-50 w-full px-[0.5rem] pt-[0.5rem] bg-transparent">
        <div class="flex justify-between w-full font-sans bg-transparent rounded-t-xl">
            <div class="px-[100px] py-3 rounded-tr-full w-full">
                <img src="/rta.png" alt="Mbadox45" class="p-1 border-gray-50" width="60px">
            ...
            <div class="flex items-center justify-center w-full p-3">
                <div class="flex items-center justify-center w-full bg-[#9a7d0a] border-4 border-[#9a7d0a] rounded-2xl">
                    <div class="flex items-center justify-center w-full">
                        <span class="flex justify-center w-full px-5 py-2 font-bold text-gray-500 transition-all duration-300 cursor-pointer rounded-xl hover:bg-white hover:text-[#9a7d0a]" v-for="(item, index) in menubar" :key="index" @click="checkActiveRoute(item.routes)" :class="route.path == item.routes ? 'bg-white text-[#9a7d0a]' : 'bg-transparent text-white'">{{item.name}}</span>
                    </div>
                </div>
            </div>
