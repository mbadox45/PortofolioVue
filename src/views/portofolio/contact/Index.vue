...
<template>
    <div class="flex items-center w-full bg-transparent rounded-b-xl gap-10 px-[4rem] pt-[7rem]">
        <!-- Title & Send Message -->
        <div class="flex flex-col items-center justify-center w-full gap-5 p-8 bg-black rounded-xl box-contact">
            <span class="text-3xl font-black text-white uppercase ">{{title}}</span>
            <p class="text-xl text-center text-yellow-300 text-md">{{pesan}}</p>
        ...
        <div class="flex flex-col justify-center w-full gap-[4rem] p-16">
            <div class="flex items-center w-full gap-5" v-for="(item, index) in contact" :key="index">
                <div :class="`flex items-center justify-center p-5 bg-black rounded-full icon-view`">
                    <i :class="`text-white pi ${item.icon}`" style="font-size:30px;"></i>
                </div>
                <div class="flex flex-col w-full gap-2 name-section">
                    <span class="text-xl font-bold underline text-[#1C325B]">{{item.name}}</span>
                    <p class="w-full text-2xl font-bold text-black">{{item.detail}}</p>
                </div>
            </div>
        </div>
        <!-- Description -->
